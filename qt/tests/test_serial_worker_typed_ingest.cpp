...
private slots:
    ...
    void typedModeParsesTypedFramesWithoutLegacyFrames() {
        ...
        QSignalSpy legacySpy(&worker, &SerialWorker::framesReceived);
        ...
        QCOMPARE(typedSpy.size(), 1);
        QCOMPARE(legacySpy.size(), 0);
        QVERIFY(statusSpy.size() >= 1);

        const auto records = qvariant_cast<TypedRecordList>(typedSpy.takeFirst().at(0));
        QCOMPARE(records.size(), 1);
        QCOMPARE(records.first().header.seq, quint16(10));
        QCOMPARE(records.first().header.recordType, static_cast<quint8>(TypedRecordType::CanRxRaw));

        const auto can = decodeTypedCanRaw(records.first());
        QVERIFY(can.has_value());
        QCOMPARE(can->monoUs, quint64(5000));
        QCOMPARE(can->bus, quint8(2));
    ...
    void typedModeReportsCrcFailureAndRecovers() {
        ...
        const auto records = qvariant_cast<TypedRecordList>(typedSpy.takeFirst().at(0));
        QCOMPARE(records.size(), 1);
        QCOMPARE(records.first().header.seq, quint16(21));

...
if exist "%BUILD_DIR%\run_release_here.bat" copy /y "%BUILD_DIR%\run_release_here.bat" "%OUTPUT%\" >nul
if exist "%BUILD_DIR%\deploy_release.bat" copy /y "%BUILD_DIR%\deploy_release.bat" "%OUTPUT%\" >nul
...
echo [5/5] 실행 도움 배치 생성
if not exist "%OUTPUT%\run_release_here.bat" (
  > "%OUTPUT%\run_release_here.bat" echo @echo off
  >> "%OUTPUT%\run_release_here.bat" echo pushd "%%~dp0"
  >> "%OUTPUT%\run_release_here.bat" echo can_monitor_qml_reboot.exe
  >> "%OUTPUT%\run_release_here.bat" echo popd
)

echo.
echo 완료: %OUTPUT%
echo 권장 실행: run_release_here.bat
endlocal
exit /b 0

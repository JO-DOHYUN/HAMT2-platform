...
constexpr int kControlMinBurstIntervalMs = 16;
constexpr int kTypedEvidenceUiMinIntervalMs = 120;
constexpr double kControlKeyboardSteerStepDeg = 2.0;
constexpr double kControlKeyboardSteerLimitDeg = 45.0;
...
bool isTypedReplayContainerPath(const QString& path) {
    const QFileInfo info(path);
    return info.isDir() || info.fileName().compare(QStringLiteral("capture.stream"), Qt::CaseInsensitive) == 0;
}

QString typedReplayStreamPathFor(const QString& path) {
    const QFileInfo info(path);
    if (info.isDir()) {
        const QString streamPath = QDir(info.absoluteFilePath()).filePath(QStringLiteral("capture.stream"));
        return QFileInfo::exists(streamPath) ? QDir::fromNativeSeparators(streamPath) : QString();
    }
    if (info.isFile() && info.fileName().compare(QStringLiteral("capture.stream"), Qt::CaseInsensitive) == 0) {
        return QDir::fromNativeSeparators(info.absoluteFilePath());
    }
    return {};
}

QString replayMetaPathFor(const QString& path) {
    const QFileInfo info(path);
    if (info.isDir()) {
        return QDir(info.absoluteFilePath()).filePath(QStringLiteral("session.meta.json"));
    }
    if (info.fileName().compare(QStringLiteral("capture.stream"), Qt::CaseInsensitive) == 0) {
        return info.dir().filePath(QStringLiteral("session.meta.json"));
    }
    return info.dir().filePath(info.completeBaseName() + QStringLiteral(".meta.json"));
}

QString normalizeTransportModeKey(const QString& raw) {
...
SerialWorker::TransportMode serialTransportModeForKey(const QString& key) {
    return normalizeTransportModeKey(key) == QStringLiteral("typed")
...
void AppController::flushPendingLiveFrames() {
    ...
    viewChunk.reserve(m_liveViewChunk);
    int processed = 0;
    while (m_pendingLiveFrameOffset < m_pendingLiveFrames.size()) {
        const FrameRecord& fr = m_pendingLiveFrames[m_pendingLiveFrameOffset++];
        ensureTimeAnchorForFrame(QStringLiteral("live"), fr.tExtUs);
        ingestFrame(fr, QStringLiteral("live"));
        viewChunk.push_back(fr);
        if (int(viewChunk.size()) > m_liveViewChunk) {
            viewChunk.remove(0, int(viewChunk.size()) - m_liveViewChunk);
            m_liveSampledViewDrops += 1;
        }
        ++processed;
        if (processed >= m_liveFlushChunk) break;
        if (processed >= m_liveFlushMinChunk && budget.elapsed() >= m_liveFlushBudgetMs) break;
    }
    compactPendingLiveFrames();

    if (!viewChunk.isEmpty()) {
        QStringList timeTexts;
        timeTexts.reserve(viewChunk.size());
        for (const FrameRecord& fr : viewChunk) {
            timeTexts << timeTextForSourceUs(QStringLiteral("live"), fr.tExtUs);
        }
        queueLiveViewBatch(viewChunk, timeTexts);
    }

    if (pendingLiveFrameCount() > 0) m_liveFlushTimer.start(pendingLiveFrameCount() > m_liveFlushChunk ? 0 : 8);
}
...
void AppController::queueLiveViewBatch(const FrameRecordList& frames, const QStringList& timeTexts) {
    ...
    if (excess > 0) {
        m_pendingLiveViewFrames.remove(0, excess);
        m_pendingLiveViewTimeTexts = m_pendingLiveViewTimeTexts.mid(excess);
    }
...
void AppController::flushQueuedLiveViewBatch() {
    ...
    FrameRecordList batch = m_pendingLiveViewFrames;
    QStringList timeTexts = m_pendingLiveViewTimeTexts;
    m_pendingLiveViewFrames.clear();
    m_pendingLiveViewTimeTexts.clear();
...
void AppController::loadReplayTimeMeta(const QString& replayBinPath) {
    ...
    const QString metaPath = replayMetaPathFor(replayBinPath);
...
AppController::AppController(QObject* parent) : QObject(parent) {
    ...
    connect(&m_boardHealthWatchdogTimer, &QTimer::timeout, this, [this]() {
        ...
        const bool previousBoardAlive = m_boardConnectionState.boardAlive();
        const bool previousControlCapable = m_boardConnectionState.controlCapable();
        m_boardConnectionState.advanceWallTimeMs(quint64(QDateTime::currentMSecsSinceEpoch()));
        const bool boardAliveChanged = previousBoardAlive != m_boardConnectionState.boardAlive();
        const bool controlCapableChanged = previousControlCapable != m_boardConnectionState.controlCapable();
        if (m_controlArmed && !m_boardConnectionState.controlCapable()) {
            m_controlArmed = false;
    ...
    m_controlKeepaliveTimer.setInterval(20);
    ...
    connect(m_worker, &SerialWorker::stateChanged, this, [this](bool ok, const QString& msg) {
        ...
        m_connected = ok;
        m_boardConnectionState.setSerialOpen(ok);
        if (ok) m_boardConnectionState.advanceWallTimeMs(quint64(QDateTime::currentMSecsSinceEpoch()));
        if (!ok) {
            m_controlArmed = false;
            m_controlTestRunning = false;
            m_controlKeepaliveTimer.stop();
    ...
    connect(m_worker, &SerialWorker::typedRecordsReceived, this, [this](const TypedRecordList& records) {
        ...
        FrameRecordList liveCanRxFrames;
        liveCanRxFrames.reserve(records.size());
        for (const auto& record : records) {
            ...
            if (can) {
                ...
                if (can->txAudit) {
                    ...
                    if (isControlCommandCanId(can->canId)) {
                        quint32 matchedCommandId = 0;
                        auto pendingIt = m_controlAcceptedAckByCanId.find(can->canId);
                        if (pendingIt != m_controlAcceptedAckByCanId.end() && !pendingIt.value().isEmpty()) {
                            matchedCommandId = pendingIt.value().takeFirst();
                            if (pendingIt.value().isEmpty()) m_controlAcceptedAckByCanId.erase(pendingIt);
                        }
                        if (matchedCommandId > 0) {
                            ++m_controlTxAuditMatchedCount;
                        } else {
                            ++m_controlTxAuditUnmatchedCount;
                        }
                        const bool auditUiDue = matchedCommandId == 0
                            || m_controlLastAuditUiWallMs <= 0
                            || (qint64(nowWallMs) - m_controlLastAuditUiWallMs) >= 250;
                        if (auditUiDue) {
                            m_controlLastAuditUiWallMs = qint64(nowWallMs);
                            m_controlLastAuditSummary = QStringLiteral("AUDIT %1 %2")
                    ...
                    if (isControlCommandCanId(can->canId)) {
                        ...
                        const qint64 lastWallMs = m_controlFeedbackLastWallMsByCanId.value(can->canId, 0);
                        if (nowWallMs - lastWallMs >= 250) {
                            m_controlFeedbackLastWallMsByCanId[can->canId] = nowWallMs;
                            ++m_controlFeedbackObservedCount;
            ...
            if (ack) {
                if (ack->status != 0 && isControlCommandCanId(ack->targetCanId)) {
                    m_controlAcceptedAckByCanId[ack->targetCanId].push_back(ack->commandId);
                }
                if (ack->status == 0) {
                    ++m_controlAckRejectedCount;
                } else {
                    ++m_controlAckAcceptedCount;
                }
                const bool ackUiDue = ack->status == 0
                    || m_controlLastAckUiWallMs <= 0
                    || (qint64(nowWallMs) - m_controlLastAckUiWallMs) >= 250;
                if (ackUiDue) {
                    m_controlLastAckUiWallMs = qint64(nowWallMs);
                    m_controlLastAckSummary = QStringLiteral("ACK #%1 %2 reason %3 BUS %4 %5 %6%7")
                        .arg(ack->commandId)
            ...
            if (boardEvent && (boardEvent->code == 12 || boardEvent->code == 17)) {
                const QString detailHex = QStringLiteral("0x%1")
                    ...
                    .toUpper();
                m_controlLastAuditSummary = QStringLiteral("NO CAN_TX_RAW: %1 detail 0x%2 counter %3")
            ...
            if (health) {
                m_boardConnectionState.ingestBoardHealth(*health, nowWallMs);
        ...
        if (!liveCanRxFrames.isEmpty()) {
            m_lastLiveFrameWallMs = QDateTime::currentMSecsSinceEpoch();
            m_pendingLiveFrames += liveCanRxFrames;
            if (!m_liveFlushTimer.isActive()) {
                m_liveFlushTimer.start(pendingLiveFrameCount() > m_liveFlushChunk ? 0 : 6);
            }
        }
    ...
    connect(m_worker, &SerialWorker::hostFrameWriteResult, this, [this](bool ok, const QString& summary, quint64 bytesWritten) {
        if (ok) {
            ++m_controlHostWriteOkCount;
        } else {
            ++m_controlHostWriteFailCount;
        }
        const bool routineWrite = ok
            && (summary.contains(QStringLiteral("worker 20ms cycle")) ||
        ...
        const QString writeSummary = QStringLiteral("%1 | serial %2 | %3 bytes")
            ...
            .arg(bytesWritten);
        if (!routineWrite) {
            m_controlLastCommandSummary = writeSummary;
    ...
    QMetaObject::invokeMethod(m_worker, [this]() {
        m_worker->setTransportMode(serialTransportModeForKey(m_transportModeKey));
    }, Qt::QueuedConnection);
    refreshPorts();
    restoreSessionState();
    updateReplayCursor(0, m_replay.frameCount(), 0, m_replay.durationUs(), 0.0);
...
AppController::~AppController() {
    saveSessionState();
    m_session.sync();
    QMetaObject::invokeMethod(m_worker, &SerialWorker::stop, Qt::QueuedConnection);
    m_workerThread.quit();
    m_workerThread.wait();
}
...
bool AppController::projectionBackpressureActive() const {
    const qint64 backlog = pendingLiveFrameCount();
    if (backlog > m_liveFlushChunk) return true;
...
QString AppController::activeViewStateSummary() const {
    const QString sourceKey = activeAnalysisSourceKey();
    const QString frameFilter = sourceKey == QStringLiteral("replay") ? m_replayFrameView.idFilter() : m_liveFrameView.idFilter();
    const QString framePart = frameFilter.isEmpty() ? QStringLiteral("프레임 필터 없음") : QStringLiteral("프레임 필터 %1").arg(frameFilter);
    return QStringLiteral("%1 · %2").arg(framePart, viewStateSummaryFor(viewStateForSource(sourceKey)));
}
...
QString AppController::logActionText() const {
    ...
    if (m_logRecordingActive) {
        QStringList parts;
        parts << QStringLiteral("현재 수신 프레임을 임시 BIN 로그로 기록 중입니다.");
    ...
    return QStringLiteral("포트를 연결한 뒤 로그 시작을 누르면 임시 BIN 로그 기록을 시작합니다.");
}

QString AppController::defaultTempLogDirectory() const {
    return QDir(defaultLogDirectory()).filePath(QStringLiteral("CAN_Monitor_Reboot_temp"));
}
...
void AppController::setTransportMode(const QString& mode) {
    ...
    QMetaObject::invokeMethod(m_worker, [this, key]() {
        m_worker->setTransportMode(serialTransportModeForKey(key));
    }, Qt::QueuedConnection);
...
QString AppController::controlStatusSummary() const {
    QStringList parts;
    parts << (m_controlArmed ? QStringLiteral("제어 ARM") : QStringLiteral("대기"));
    parts << (m_controlTestRunning ? QStringLiteral("테스트 실행 중") : QStringLiteral("수동"));
    parts << QStringLiteral("BUS %1").arg(m_controlTargetBus);
    parts << QStringLiteral("목표 rpm %1").arg(m_controlTargetRpm);
    parts << QStringLiteral("목표 조향 %1 deg").arg(QString::number(m_controlTargetSteeringDeg, 'f', 1));
    parts << QStringLiteral("출력 제한 조향 1.0deg/20ms, rpm 250/20ms");
    if (m_transportModeKey != QStringLiteral("typed")) parts << QStringLiteral("typed 모드 필요");
    if (!m_connected) parts << QStringLiteral("미연결");
    if (m_transportModeKey == QStringLiteral("typed") && m_connected && !controlEvidenceReady()) {
        parts << QStringLiteral("보드 gate: %1").arg(controlEvidenceBlockReason());
    }
    if (!m_controlStatusText.isEmpty()) parts << m_controlStatusText;
    return parts.join(QStringLiteral(" | "));
}

QString AppController::controlEvidenceStatsSummary() const {
    quint64 pendingAuditCount = 0;
    for (auto it = m_controlAcceptedAckByCanId.cbegin(); it != m_controlAcceptedAckByCanId.cend(); ++it) {
        pendingAuditCount += quint64(it.value().size());
    }
    return QStringLiteral("요청 %1 | Qt write OK/Fail %2/%3 | ACK OK/Reject %4/%5 | TX audit 대기/매칭/미매칭 %6/%7/%8 | feedback %9")
        .arg(m_controlHostFrameQueuedCount)
        .arg(m_controlHostWriteOkCount)
        .arg(m_controlHostWriteFailCount)
        .arg(m_controlAckAcceptedCount)
        .arg(m_controlAckRejectedCount)
        .arg(pendingAuditCount)
        .arg(m_controlTxAuditMatchedCount)
        .arg(m_controlTxAuditUnmatchedCount)
...
void AppController::appendControlEvidenceEvent(const QString& stage,
    ...
    QVariantMap row;
    row.insert(QStringLiteral("key"), QStringLiteral("%1").arg(++m_controlEvidenceEventCounter, 12, 10, QLatin1Char('0')));
    ...
    m_controlEvidenceRows.push_back(row);
    while (m_controlEvidenceRows.size() > 80) {
        m_controlEvidenceRows.removeFirst();
    }
    m_controlEvidenceModel.setRows(m_controlEvidenceRows);
}

bool AppController::controlEvidenceReady() const {
    return m_boardConnectionState.controlCapable() && controlTargetBusAllowed();
}

QString AppController::controlEvidenceBlockReason() const {
    if (!m_boardConnectionState.controlCapable()) return m_boardConnectionState.reason();
    if (!controlTargetBusAllowed()) {
        return QStringLiteral("target bus %1 is not resolved/allowed for control TX (%2)")
            .arg(m_controlTargetBus)
            .arg(controlBusResolutionSummary());
    }
    return m_boardConnectionState.reason();
}

bool AppController::controlTargetBusAllowed() const {
    if (m_controlCapabilityHasBusDescriptors && !m_controlTxAllowedBuses.contains(m_controlTargetBus)) {
        return false;
    }

    const auto resolution = m_busRoleResolver.resolve(quint8(m_controlTargetBus));
...
void AppController::seedBusRoleResolver() {
    ...
    if (m_controlTargetBusManualOverride) {
        m_busRoleResolver.setOperatorOverride(quint8(m_controlTargetBus), QStringLiteral("system"), true);
    }
}

void AppController::observeBusRoleFingerprint(quint8 bus, quint32 canId) {
    if (!isSystemBusFingerprint(canId)) return;
    ...
    m_busRoleResolver.observeCanId(bus, canId);
...
bool AppController::autoSelectSystemControlBus() {
    if (m_controlTargetBusManualOverride) return false;
    ...
    for (auto it = m_systemFingerprintHitsByBus.cbegin(); it != m_systemFingerprintHitsByBus.cend(); ++it) {
        ...
        if (!resolution.resolved || resolution.role != QStringLiteral("system") || !resolution.txAllowed) {
            continue;
        }
        if (bestBus < 0 || it.value() > bestHits || (it.value() == bestHits && bus == m_controlTargetBus)) {
            bestBus = bus;
            bestHits = it.value();
        }
    }

    if (bestBus < 0 || bestBus == m_controlTargetBus) return false;
    m_controlTargetBus = bestBus;
    m_controlStatusText = QStringLiteral("Auto-selected system CAN bus %1 from observed evidence").arg(bestBus);
    return true;
}
...
QString AppController::controlBusResolutionSummary() const {
    ...
    if (!resolution.resolved) {
        return QStringLiteral("bus%1 unresolved").arg(m_controlTargetBus);
    }
    const quint64 hits = m_systemFingerprintHitsByBus.value(quint8(m_controlTargetBus));
    QStringList parts;
    parts << QStringLiteral("bus%1 role %2").arg(m_controlTargetBus).arg(resolution.role);
    ...
    if (m_controlTargetBusManualOverride) parts << QStringLiteral("manual");
    if (m_controlCapabilityHasBusDescriptors) {
        parts << QStringLiteral("cap%1").arg(m_controlTxAllowedBuses.contains(m_controlTargetBus) ? 1 : 0);
    }
    return parts.join(QStringLiteral(" "));
}
...
void AppController::queueControlHostFrame(const QByteArray& frame,
                                          ...
                                          quint32 canId,
                                          quint8 bus) {
    if (!m_worker || frame.isEmpty()) return;
    ++m_controlHostFrameQueuedCount;
    if (!stage.isEmpty()) {
    ...
    QMetaObject::invokeMethod(m_worker, [this, frame, summary]() {
        m_worker->sendHostFrame(frame, summary);
    }, Qt::QueuedConnection);
}

void AppController::sendControlHeartbeat(const QString& reason) {
    if (!m_connected || m_transportModeKey != QStringLiteral("typed")) return;
    const quint32 commandId = ++m_controlCommandCounter;
    ...
    m_controlLastHeartbeatWallMs = QDateTime::currentMSecsSinceEpoch();
...
void AppController::sendControlSession(quint8 action, const QString& reason) {
    ...
    if (action == CanMonitorControl::kControlSessionArm || action == CanMonitorControl::kControlSessionRenewLease) {
        m_controlLastLeaseRenewWallMs = QDateTime::currentMSecsSinceEpoch();
    }
...
void AppController::maintainControlLeaseAndHeartbeat(bool forceHeartbeat, bool forceLeaseRenew) {
    const qint64 nowWallMs = QDateTime::currentMSecsSinceEpoch();
    if (forceHeartbeat || nowWallMs - m_controlLastHeartbeatWallMs >= 50) {
        sendControlHeartbeat(forceHeartbeat ? QStringLiteral("heartbeat prime") : QStringLiteral("heartbeat keepalive"));
    }
    if (m_controlArmed && (forceLeaseRenew || nowWallMs - m_controlLastLeaseRenewWallMs >= 250)) {
...
void AppController::setControlTargetBus(int bus) {
    ...
    if (m_controlTargetBus == nextBus) return;
    m_controlTargetBus = nextBus;
    m_controlTargetBusManualOverride = true;
    m_busRoleResolver.setOperatorOverride(quint8(m_controlTargetBus), QStringLiteral("system"), true);
...
void AppController::sendControlBurst(int signedCommand,
    ...
    const int safeRpm = std::clamp(rpm, 0, 10000);
    const double safeSteer = std::clamp(steeringDeg, -90.0, 90.0);
    const auto frames = CanMonitorControl::ControlCommandEncoder::makeControlBurst(
        ...
        motorMode,
        drivingMode,
        m_controlAliveCounter++,
        quint8(m_controlTargetBus));
    ...
    m_controlLastCommandSummary = summaries.join(QStringLiteral(" / "));
...
void AppController::setControlTargetCommand(int signedCommand,
                                            ...
                                            quint8 drivingMode,
                                            const QString& reason) {
    m_controlCurrentSignedCommand = std::clamp(signedCommand, -10000, 10000);
    m_controlCurrentRpm = std::clamp(rpm, 0, 10000);
    m_controlCurrentSteeringDeg = std::clamp(steeringDeg, -90.0, 90.0);
    m_controlCurrentMotorMode = motorMode == 2 ? 2 : 1;
    m_controlCurrentDrivingMode = drivingMode;
    m_controlStatusText = QStringLiteral("Target latched: %1").arg(reason);
    emit controlStateChanged();
    if (m_controlArmed) updateControlWorkerCycleTarget();
}

void AppController::updateControlWorkerCycleTarget() {
    if (!m_worker) return;
    const int signedCommand = m_controlCurrentSignedCommand;
    const int rpm = m_controlCurrentRpm;
    const double steeringDeg = m_controlCurrentSteeringDeg;
    const quint8 motorMode = m_controlCurrentMotorMode;
    const quint8 drivingMode = m_controlCurrentDrivingMode;
    const quint8 bus = quint8(m_controlTargetBus);
    QMetaObject::invokeMethod(m_worker, [this, signedCommand, rpm, steeringDeg, motorMode, drivingMode, bus]() {
        m_worker->updateControlCycle(signedCommand, rpm, steeringDeg, motorMode, drivingMode, bus);
    }, Qt::QueuedConnection);
}

void AppController::sendControlKeepaliveTick(bool force) {
    if (!m_controlArmed || !m_connected || m_transportModeKey != QStringLiteral("typed")) return;
    if (!controlEvidenceReady()) {
        m_controlArmed = false;
        stopControlKeepalive();
    ...
    const qint64 nowWallMs = QDateTime::currentMSecsSinceEpoch();
    if (!force && m_controlLastBurstWallMs > 0 && nowWallMs - m_controlLastBurstWallMs < kControlMinBurstIntervalMs) {
        return;
    }
    m_controlLastBurstWallMs = nowWallMs;
    if (force && m_worker) {
        const int signedCommand = m_controlCurrentSignedCommand;
        const int rpm = m_controlCurrentRpm;
        const double steeringDeg = m_controlCurrentSteeringDeg;
        ...
        const quint8 bus = quint8(m_controlTargetBus);
        QMetaObject::invokeMethod(m_worker, [this, signedCommand, rpm, steeringDeg, motorMode, drivingMode, bus]() {
            m_worker->sendControlCycleBurstOnce(signedCommand, rpm, steeringDeg, motorMode, drivingMode, bus, QStringLiteral("worker forced burst"));
        }, Qt::QueuedConnection);
    } else {
...
void AppController::startControlKeepalive() {
    ...
    const quint8 bus = quint8(m_controlTargetBus);
    QMetaObject::invokeMethod(m_worker, [this, signedCommand, rpm, steeringDeg, motorMode, drivingMode, bus]() {
        m_worker->startControlCycle(signedCommand, rpm, steeringDeg, motorMode, drivingMode, bus, 20, kControlBurstFrameGapMs);
    }, Qt::QueuedConnection);
    m_controlLastBurstWallMs = QDateTime::currentMSecsSinceEpoch();
}
...
void AppController::stopControlKeepalive() {
    ...
    if (!m_worker) return;
    QMetaObject::invokeMethod(m_worker, [this]() {
        m_worker->stopControlCycle();
    }, Qt::QueuedConnection);
}

void AppController::controlKeyboardCommand(const QString& key) {
    const QString normalized = key.trimmed().toLower();
    const int rpm = m_controlTargetRpm;
    double steer = m_controlTargetSteeringDeg;
    quint8 motorMode = 0;
    quint8 drivingMode = 2;
    int signedCommand = 0;

    if (normalized == QStringLiteral("w")) {
        motorMode = 1;
        signedCommand = rpm;
        steer = m_controlTargetSteeringDeg;
    } else if (normalized == QStringLiteral("s")) {
        motorMode = 1;
        signedCommand = -rpm;
        steer = m_controlTargetSteeringDeg;
    } else if (normalized == QStringLiteral("a")) {
        steer = std::clamp(m_controlTargetSteeringDeg - kControlKeyboardSteerStepDeg,
                           -kControlKeyboardSteerLimitDeg,
                           kControlKeyboardSteerLimitDeg);
        motorMode = 1;
        signedCommand = m_controlCurrentSignedCommand != 0 ? m_controlCurrentSignedCommand : rpm;
    } else if (normalized == QStringLiteral("d")) {
        steer = std::clamp(m_controlTargetSteeringDeg + kControlKeyboardSteerStepDeg,
                           -kControlKeyboardSteerLimitDeg,
                           kControlKeyboardSteerLimitDeg);
        motorMode = 1;
    ...
    m_controlTargetSteeringDeg = steer;
    setControlTargetCommand(signedCommand, rpm, steer, motorMode, drivingMode, QStringLiteral("WSAD %1").arg(normalized.toUpper()));
    sendControlKeepaliveTick();
}

void AppController::controlSendManual() {
    const int rpm = m_controlTargetRpm;
    setControlTargetCommand(rpm, rpm, m_controlTargetSteeringDeg, 1, 2, QStringLiteral("manual"));
    sendControlKeepaliveTick();
}
...
void AppController::controlSendNeutral() {
    ...
    m_controlPatternSteps.clear();
    m_controlPatternIndex = 0;
    setControlTargetCommand(0, 0, 0.0, 1, 1, QStringLiteral("neutral"));
    sendControlKeepaliveTick(true);
}
...
void AppController::processControlPatternStep() {
    if (!m_controlTestRunning || m_controlPatternSteps.isEmpty()) return;
    ...
    m_controlTargetRpm = step.rpm;
    m_controlTargetSteeringDeg = step.steeringDeg;
    setControlTargetCommand(signedCommand, step.rpm, step.steeringDeg, step.motorMode, step.drivingMode, step.label);
    sendControlKeepaliveTick();
    if (m_controlTestRunning) m_controlPatternTimer.start(std::max(120, step.holdMs));
}
...
void AppController::connectPort(const QString& portName) {
    ...
    QMetaObject::invokeMethod(m_worker, [this, trimmed]() {
        m_worker->start(trimmed);
    }, Qt::QueuedConnection);
}
...
void AppController::startLog() {
    ...
    if (m_transportModeKey == QStringLiteral("typed")) {
        const QString sessionDir = QDir(defaultLogDirectory()).filePath(QStringLiteral("typed_capture_%1.typed").arg(stamp));
        ...
        m_logTypedSession = true;
        m_logTempPath = sessionDir;
        m_logTempMetaPath.clear();
        m_logTempModelPath.clear();
        m_logSuggestedSavePath = sessionDir;
        m_logRecordedBytes = 0;
        m_logRecordedFrameCount = 0;
        m_logStopping = false;
        m_logSaving = false;
        m_logPendingSave = false;
        m_logPath = sessionDir;
        emit logPathChanged();
        requestLogStateRefresh(true);

        QMetaObject::invokeMethod(m_worker, [this, sessionDir, metadata]() {
            m_worker->setTypedStorage(true, sessionDir, metadata);
        }, Qt::QueuedConnection);
    ...
    m_logTempPath = QDir(tempDir).filePath(QStringLiteral("capture_%1.bin").arg(stamp));
    m_logTempMetaPath = QDir(tempDir).filePath(QStringLiteral("capture_%1.meta.json").arg(stamp));
    m_logTempModelPath = m_modelEnabled ? QDir(tempDir).filePath(QStringLiteral("capture_%1.model.json").arg(stamp)) : QString();
    m_logSuggestedSavePath = QDir(defaultLogDirectory()).filePath(QStringLiteral("can_log_%1.bin").arg(stamp));
    m_logRecordedBytes = 0;
    m_logRecordedFrameCount = 0;
    m_logStopping = false;
    m_logSaving = false;
    ...
    QMetaObject::invokeMethod(m_worker, [this, rulesPath]() {
        m_worker->setLogging(true, m_logTempPath, m_logTempMetaPath, m_logTempModelPath, rulesPath);
    }, Qt::QueuedConnection);
...
void AppController::resetTypedEvidenceState() {
    ...
    seedBusRoleResolver();
    m_boardConnectionState.reset();
    m_boardConnectionState.setSerialOpen(m_connected);
    m_controlCapabilityHasBusDescriptors = false;
    m_controlTxAllowedBuses.clear();
    m_controlBusSummary = QStringLiteral("waiting for CAPABILITY bus descriptors");
    m_controlLastAckSummary = QStringLiteral("No CONTROL_ACK yet");
    m_controlLastAuditSummary = QStringLiteral("No CAN_TX_RAW audit yet");
    m_controlEvidenceRows.clear();
    m_controlEvidenceEventCounter = 0;
    ...
    m_controlTxAuditUnmatchedCount = 0;
    m_controlFeedbackObservedCount = 0;
    m_controlLastAckUiWallMs = 0;
    m_controlLastAuditUiWallMs = 0;
    m_controlAcceptedAckByCanId.clear();
...
void AppController::stopLog() {
    ...
    if (m_logTypedSession) {
        QMetaObject::invokeMethod(m_worker, [this]() {
            m_worker->setTypedStorage(false, m_logTempPath, QJsonObject{});
        }, Qt::QueuedConnection);
        setStatus(QStringLiteral("Typed capture finalize requested"));
        return;
    }
    QMetaObject::invokeMethod(m_worker, [this]() {
        m_worker->setLogging(false, QString(), QString(), QString(), QString());
    }, Qt::QueuedConnection);
    setStatus(QStringLiteral("로그 종료 중 · 임시 버퍼 정리"));
}

void AppController::finalizePendingLogSave(const QString& filePath) {
    const QString normalized = normalizeLocalPath(filePath);
    if (normalized.isEmpty()) {
        setStatus(QStringLiteral("로그 저장이 취소되었습니다"));
        return;
    }
    const QString finalBin = normalized.endsWith(QStringLiteral(".bin"), Qt::CaseInsensitive) ? normalized : (normalized + QStringLiteral(".bin"));
    const QString base = finalBin.left(finalBin.size() - 4);
    const QString finalMeta = base + QStringLiteral(".meta.json");
    const QString finalModel = base + QStringLiteral(".model.json");
...
void AppController::loadReplay(const QString& filePath) {
    ...
    if (isTypedReplayContainerPath(normalized)) {
        const QString streamPath = typedReplayStreamPathFor(normalized);
        if (streamPath.isEmpty()) {
            setReplayLoaded(false);
            setReplayPlaying(false);
            setStatus(QStringLiteral("Typed replay session has no capture.stream: %1").arg(normalized));
            return;
        }
        ...
        QString error;
        if (!reader.loadFile(streamPath, &error)) {
            setReplayLoaded(false);
            setReplayPlaying(false);
...
void AppController::playReplay(double speed) {
    ...
    clearReplaySeekState();
    m_replaySpeed = (speed <= 0.0) ? 1.0 : speed;
    setReplayAnalysisHeld(true);
    const int frameCount = m_replay.frameCount();
    if (frameCount > 0) {
